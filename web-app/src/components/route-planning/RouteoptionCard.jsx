...
const RouteOptionCard = ({ route, selected, onSelect }) => (
  ...
  >
    ...
    <div className="flex gap-3 mt-2 text-xs">
      <span className={`${route.risk === 'Low' ? 'text-green-600' : route.risk === 'Medium' ? 'text-yellow-600' : 'text-orange-600'}`}>
        ⚠️ Risk: {route.risk}
      </span>
      <span className="text-green-600 dark:text-green-400">💰 ${route.cost}</span>
    </div>
  </div>
);
