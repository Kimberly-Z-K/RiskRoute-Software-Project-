...
const generateDescription = (
  ...
  switch (action) {
    ...
    case 'CLICK': {
      const elementName =
        element?.replace(/[^-]+-\s*/, '').trim() || 'a button';

      return `${user} clicked on "${elementName}" on the ${page} page`;
    }

    case 'CREATE':
      return `${user} created a new ${targetName || 'record'}${
    ...
    case 'UPDATE':
      if (beforeValue && afterValue) {
        return `${user} updated ${
          targetName || 'a record'
        } from "${beforeValue}" to "${afterValue}" on the ${page} page`;
      }
    ...
    case 'COMPLETE':
      return `${user} marked ${
        targetName || 'a delivery'
      } as completed`;

    case 'CANCEL':
      return `${user} cancelled ${
        targetName || 'an operation'
      } on the ${page} page`;

    case 'VIEW_DETAILS':
      return `${user} viewed details of ${
        targetName || 'a record'
      } on the ${page} page`;
...
const updateVehicleSpeed = (currentSpeed) => {
  ...
  return Math.max(
    0,
    Math.min(120, (currentSpeed || 60) + change)
  );
};
...
const updateDriverScore = (currentScore) => {
  ...
  return Math.max(
    60,
    Math.min(100, (currentScore || 85) + change)
  );
};
...
const DashboardContent = React.memo(
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                      ...
                      <KPICard
                        ...
                        value={
                          performanceData.driverSafetyScore ||
                          88
                        }
                        change={2}
                        icon={Shield}
                      ...
                      <KPICard
                        ...
                        value={
                          Math.abs(
                            performanceData.fuelUsageTrend
                          ) || 5
                        }
                        unit="%"
                        change={
                          performanceData.fuelUsageTrend || 0
                        }
                        icon={Fuel}
                        color="bg-yellow-500"
                      ...
                      <KPICard
                        ...
                        value={
                          performanceData.riskIncidentFrequency ||
                          12
                        }
                        change={-21}
                        icon={AlertTriangle}
  ...
  (prevProps, nextProps) => {
    return (
      ...
      prevProps.isRealTimeActive ===
        nextProps.isRealTimeActive
    );
  }
);
      ...
      setTimeout(() => {
        ...
        auditLog({
          ...
          description: `${
            ...
            userData.name ||
            userData.email
          }) logged into the fleet management system`,
          details: {
            method:
              'email_password'
          }
        ...
        try {
          ...
          auditLog({
            ...
            description: `${
              ...
              user.name ||
              user.email
            }) logged out of the fleet management system`,
            details: {
              logout_time:
    ...
    statsUpdateTimeout.current =
      setTimeout(() => {
        ...
        if (currentIds !== prevIds) {
          ...
          console.log(
            '⏭️ Skipping stats update - no vehicle changes'
          );
    ...
    useCallback(
      (id) => {
        ...
        auditLog({
          ...
          details: {
            alertType:
              alertToAcknowledge?.type ||
              null,
            priority:
              alertToAcknowledge?.priority ||
              null
          }
        });
    ...
    useCallback(
      () => {
        ...
        if (routes.length === 0) {
          ...
          alert(
            'Please wait for routes to load from the database'
          );
        ...
        const accidentImpact =
          simulationParams.accident
            ? 35
            : 0;

        const roadClosureImpact =
          simulationParams.roadClosure
            ? 28
            : 0;
        ...
        const baseTime =
          currentRoute?.estimated_time ||
          30;

        const baseCost =
          currentRoute?.estimated_cost ||
          10;
        ...
        auditLog({
          ...
          targetName:
            currentRoute?.name ||
            'Current Route',
          description: `User ran a what-if simulation using the ${
            currentRoute?.name ||
            'current route'
          }`,
          details: {
